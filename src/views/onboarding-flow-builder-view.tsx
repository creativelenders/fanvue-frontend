...
import { useOnboardingFunnel, useUpdateOnboardingFunnel } from "../hooks/use-automation";
...
const TriggerNode = ({ data }: { data: any }) => (
  <div className="bg-glass-base border-2 border-primary-cyan rounded-lg p-3 min-w-[200px] shadow-lg shadow-primary-cyan/20">
    <div className="flex items-center gap-2 mb-2">
      ...
      <span className="text-sm font-semibold text-text-main">New Subscriber</span>
    </div>
    <div className="mt-2 text-xs text-text-muted">
      Triggers when a fan subscribes.
    </div>
...
const initialNodes = [
  { id: '1', type: 'triggerNode', position: { x: 250, y: 50 }, data: {} },
  { id: '2', type: 'messageNode', position: { x: 250, y: 200 }, data: { message: 'Welcome! ❤️' } },
];
const initialEdges = [{ id: 'e1-2', source: '1', target: '2' }];

export function OnboardingFlowBuilderView() {
  ...
  useEffect(() => {
    if (funnel) {
      if (funnel.graphData && funnel.graphData.nodes) {
        setNodes(funnel.graphData.nodes);
        setEdges(funnel.graphData.edges);
      } else {
        setNodes(initialNodes);
        setEdges(initialEdges);
      }
    }
  }, [funnel, setNodes, setEdges]);
  ...
  if (error || !funnel) return <ErrorState onRetry={() => refetch()} />;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-bold text-text-main">{funnel.name}</h1>
              <StatusBadge status={funnel.isActive ? "active" : "paused"} />
            </div>
            <p className="text-sm text-text-muted mt-1">{funnel.description}</p>
          </div>
        </div>
